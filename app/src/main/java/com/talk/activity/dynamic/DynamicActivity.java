...
public class DynamicActivity extends BaseActivity<Dynamicstract.Presenter> implements DynamicPublishAdapter.OpenPhoto, Dynamicstract.View {
    ...
    protected void initData() {
        ...
        //判断上次是否有保存的草稿信息
        String username = SpUtils.getInstance().getString("usename");
        if (!TextUtils.isEmpty(username)) {//有存进sp账号
            //将存进账号里的json串解析出来
            DynamicSaveDataBean bean = new Gson().fromJson(username, DynamicSaveDataBean.class);
